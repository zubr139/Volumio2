...
ControllerMpd.prototype.explodeUri = function(uri) {
    ...
    else if(uri.startsWith('years://artists')) {
        /*
         years://artists/year/artist in service mpd
         */
        var splitted = uri.split('/');

        var artist = nodetools.urlDecode(splitted[4]);
        ...
        self.clientMpd.sendCommand(cmd("find artist \""+artist+"\"", []), function (err, msg) {
            if(msg=='')
            {
                self.clientMpd.sendCommand(cmd("find albumartist \""+artist+"\"", []), function (err, msg) {
                    self.exploderArtistByDate(err,msg,defer,splitted[3]);
                });
            }
            else self.exploderArtistByDate(err,msg,defer,splitted[3]);
        });
    }
...
ControllerMpd.prototype.exploderArtistByDate=function(err,msg,defer,year)
{
    var self=this;
    ...
    if (msg) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line.indexOf('file:') === 0) {
                ...
                var album = self.searchFor(lines, i + 1, 'Album:');
                var title = self.searchFor(lines, i + 1, 'Title:');
                var itemYear= moment(self.searchFor(lines, i+1, 'Date:'));

                if(yearAsDate.isSame(itemYear))
                {
                    var albumart=self.getAlbumArt({artist: artist, album: album}, self.getParentFolder('/mnt/'+path));
                    var time = parseInt(self.searchFor(lines, i + 1, 'Time:'));
...
ControllerMpd.prototype.handleBrowseUri = function (curUri) {
    ...
    if (curUri.startsWith('music-library')) {
    ...
    }else if (curUri.startsWith('years://')) {
        if (curUri == 'years://')
        ...
        else
        {
            ...
            {
                response = self.listYearsForItem(curUri,2,'years://');
            }
            else{
                var type=splitted[2];
                var year=splitted[3];
                ...
                if(type=='albums')
                {
                    //years://albums/1977/pino
                    if(splitted.length==4)
                    {
                        response = self.listItemsForYear(curUri,2,3,'years://albums');
                    }
                    else
                    {
                ...
                else if(type=='artists')
                {
                    if(splitted.length==4)
                    {
                        response = self.listItemsForYear(curUri,2,3,'years://artists');
                    }
                    else
                    {
                        response = self.listArtistByYear(curUri,4,'years://artists/'+splitted[3],undefined,splitted[3]);
                    }
                }
                else
                {
                    if(splitted.length==4)
                    {
                        response = self.listItemsForYear(curUri,2,3,'years://songs');
                    }
                    else
                    {
                    }
...
ControllerMpd.prototype.listYears = function () {
    var response = {
        "navigation": {
            "lists": [
                {
...
ControllerMpd.prototype.listItemsForYear = function (curUri, index, dateIndex, previous) {
    var self = this;
    ...
    var dateToSearch=moment(splitted[dateIndex]).format('YYYY');
    var itemnos;
    var lineStr;
    var album;
    var artist;
    ...
    self.clientMpd.sendCommand(cmd("list", [itemnos,"group","date"]), function (err, msg) {
        if(err)
            defer.reject(new Error('Cannot list '+itemnos));
        else
        {
            var lines=msg.split('\n');
            ...
            for (var i = 0; i < lines.length; i++)
            {
                ...
                if (line.indexOf(lineStr) === 0) {
                    var date = moment(self.searchFor(lines, i + 1, 'Date:').trim()).format('YYYY');
            ...
            var cleanedArray=_.without(yearsList,undefined, '');
            var uniqueArray=_.uniq(cleanedArray);
            var sortedArray=_.sortBy(uniqueArray,function(num){ return num; });
        ...
        else
        {
            ...
            for (var i = 0; i < lines.length; i++)
            {
                ...
                if (line.indexOf('Date:') === 0) {
                    var momentDate=moment(line.slice(6).trim());

                    if(momentDate.isValid())
                    {
                        var date =momentDate.format('YYYY');
                        yearsList.push(date);
                    }
                }
            }
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (line.indexOf('file:') === 0) {
                            ...
                            var albumart = self.getAlbumArt({
                                artist: artist,
                                album: album
                            }, self.getParentFolder(path), 'fa-dot-circle-o');
...
ControllerMpd.prototype.listArtistByYear = function (curUri,index,previous,uriBegin,year) {
    var self = this;
                ...
                if (msg) {
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (line.indexOf('file:') === 0) {
                            ...
                            if(yearMoment.isSame(itemYear))
                            {
                                console.log(itemYear.format("YYYY")+"      "+yearMoment.format("YYYY"));
                    ...
                    if (msg == '') {
                        self.clientMpd.sendCommand(cmd("find albumartist \"" + artist + "\"", []), function (err, msg) {
                            self.parseListAlbumByYear(err, msg, defer, response, uriBegin,year);
                        });
                    }
                    else {
                        self.parseListAlbumByYear(err, msg, defer, response, uriBegin,year);
...
ControllerMpd.prototype.parseListAlbumByYear= function(err,msg,defer,response,uriBegin,year)
{
    var self=this;
    var list = [];
    var albums=[],albumarts=[];
